...
@register_model
def deit_tiny_patch16_224(pretrained=False, depth=12, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs):
    del kwargs['pretrained_cfg']
    model = VisionTransformer(
        patch_size=16, embed_dim=192, depth=depth, num_heads=3, mlp_ratio=4, qkv_bias=True,
...
@register_model
def deit_small_patch16_224(pretrained=False, depth=12, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs):
    del kwargs['pretrained_cfg']
    model = VisionTransformer(
        patch_size=16, embed_dim=384, depth=depth, num_heads=6, mlp_ratio=4, qkv_bias=True,
        norm_layer=norm_layer, **kwargs)
    model.default_cfg = _cfg()
    if pretrained:
        checkpoint = torch.hub.load_state_dict_from_url(
...
@register_model
def deit_base_patch16_224(pretrained=False, depth=12, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs):
    del kwargs['pretrained_cfg']
    model = VisionTransformer(
