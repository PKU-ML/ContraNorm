...
# sys.path.insert(0, "/home/xjguo/unitoken/src/")
import transformers
...
def main():
    ...
    elif args.model_name_or_path == "microsoft/deberta-base":
    ...
    total_metrics = dict()
    COLLAPSE_METRICS = ['sims', 'eranks', 'vars']
    for epoch in range(args.num_train_epochs):
        start_time = datetime.datetime.now()

        cal_svd = False if epoch == args.num_train_epochs - 1 else False
        cal_metrics = False if epoch == args.num_train_epochs - 1 else False
        end_step = 0
        erank_step = 1
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if step >= erank_step :
                cal_svd = False
            outputs = model(**batch, cal_svd=cal_svd, cal_metrics=cal_metrics)  #original is batch
            loss = outputs.loss

            if cal_metrics:
                for k, v in outputs.metrics.items():
                    if k in ['cor_s', 'eranks'] and not cal_svd:
                        continue
                    if k not in total_metrics.keys():
                        total_metrics[k] = torch.Tensor(np.array(v))
                    else:
                        total_metrics[k] += torch.Tensor(np.array(v))
        ...
        if cal_metrics:
            for k, v in total_metrics.items():
                if k in ['eranks', 'cor_s']:
                    total_metrics[k] /= erank_step
                else:
                    total_metrics[k] /= end_step
                total_metrics[k] = total_metrics[k].detach().cpu().numpy()

            with open(f'{args.output_dir}/metrics-{args.scale}-{args.pos}-{args.num_hidden_layers}-epoch{epoch+1}.pkl', 'wb') as f:
