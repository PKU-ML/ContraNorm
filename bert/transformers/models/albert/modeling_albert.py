...
class ContraNorm(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x - self.scale * torch.bmm(sim, x)
        return x
...
class AlbertAttention(nn.Module):
    ...
    def forward(
        ...
        if '2' in self.pos:
            context_layer = self.contra_norm(context_layer) 

        projected_context_layer = self.dense(context_layer)
        projected_context_layer_dropout = self.output_dropout(projected_context_layer)
        if '3' in self.pos:
            projected_context_layer_dropout = self.contra_norm(projected_context_layer_dropout) 

        if '4' in self.pos:
            layernormed_context_layer = self.LayerNorm(self.contra_norm(hidden_states + projected_context_layer_dropout))
        else:
            layernormed_context_layer = self.LayerNorm(hidden_states + projected_context_layer_dropout)

        if '5' in self.pos:
            layernormed_context_layer = self.contra_norm(layernormed_context_layer) 
...
class AlbertLayer(nn.Module):
    ...
    def forward(
        ...
        if '6' in self.pos:
            ffn_output = self.contra_norm(ffn_output)

        if '7' in self.pos:
            hidden_states = self.full_layer_layer_norm(self.contra_norm(ffn_output + attention_output[0]))
        else:
